...
This is a **static site with no build step**. `vercel.json` says so explicitly
(`framework: null`, `buildCommand: null`, `outputDirectory: "."`), and `package.json`
deliberately has **no `build` script** — if one is present, Vercel runs it and then fails
looking for a `public/` output directory that does not exist.
...
Or push the folder to a Git repo and import it in Vercel. If the dashboard asks, set:
...
If you previously imported this project and it failed, the old settings are cached on the
project — fix them under **Settings → Build & Deployment** and redeploy, or delete the
project and re-import.
